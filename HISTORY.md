...
**命名遗留**：`Gillespie.py` 系列**并不是 Gillespie 算法**，
...
整理归档时，以下两处**原始注释/写法被修改**，原始内容保留在此以备追溯：
...
原注释是"目标导向"的（"为了显示…"），读起来像是为了让图好看而调参数。
实际上参数取保守下界是合理的科研选择，但**表述方式**会让读者产生相反的理解。

### 2. `legacy/drug_efficacy_v3_simple.py`——指标名与均值算法
