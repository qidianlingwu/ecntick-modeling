...
class PulsedImmuneModel:
    def model(self, t, y):
        # y = [B, E, NB, T, A, X, J, NB_total]
        B, E, NB, T, A, X, J, NB_total = y
        ...
        return [dB, dE, dNB, dT, dA, dX, dJ, dNB_total]
...
# ==================== 运行并绘图 ====================
def run_and_plot():
    # 脉冲组初始状态：[B, E, NB, T, A, X, J, NB_total]
    y0_p = [0.08, 0.0, 0.0, 1.0, A_0, 0.0, 0.0, 0.0]
    ...
    peak_p = np.max(ys_p[:, 2])
    peak_c = np.max(ys_c[:, 0])
    avg_p = time_weighted_mean(ts_p, ys_p[:, 2])
    ...
    # 药效输出
    eff_p = drug_effect(ys_p[:, 2])
    eff_c = drug_effect(ys_c[:, 0])
    j_eff_p = adjuvant_effect(ys_p[:, 6])
    kill_p = k_kill * eff_p * (ys_p[:, 4] / (ys_p[:, 4] + A_threshold)) * (1 + eta_J * j_eff_p)
    ...
    axes[0, 0].plot(ts_p, ys_p[:, 3], 'r-', lw=2.0, label='脉冲释放')
    ...
    axes[0, 2].plot(ts_p, ys_p[:, 4], 'r-', lw=2.0, label='脉冲')
    ...
    axes[1, 0].plot(ts_p, ys_p[:, 5], 'r-', lw=2.0, label='脉冲')
